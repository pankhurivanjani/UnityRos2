#!/usr/bin/env python
#import rospkg
#install utm via pip3 install utm to run this script
...
from catkin_pkg.package import InvalidPackage, parse_package #not sure


#_, package_path = get_resource('packages', turtlebot3_unity_bringup)

json_fn = "i3.json"
resolution = 0.5
#def draw_polygon_map(pkg_path,resolution, json_filename):
def draw_polygon_map(resolution, json_filename):
#    with open(pkg_path + "/maps/" + json_filename, "r") as geo_fence_file:
    with open(json_filename, "r") as geo_fence_file:
...
#    param_file = open(pkg_path + "/maps/" + yaml_filename,"w+")
    param_file = open(yaml_filename,"w+")
...
#    centre_param_file = open(pkg_path + "/maps/" + centre_yaml_filename, "w+")
    centre_param_file = open(centre_yaml_filename, "w+")
...
draw_polygon_map(resolution, json_fn)
